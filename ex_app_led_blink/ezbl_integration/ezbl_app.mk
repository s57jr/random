...
# Create a binary .bl2 file from the .elf file and also if a loadable exists, convert he unified .hex file. The .elf file is preferred so it works in debug mode and also so we can obtain the BOOTID_HASH meta-data.
	...
	-test "dist/${CONF}/${IMAGE_TYPE}/${PROJECTNAME}.${IMAGE_TYPE}.bl2" -nt "dist/${CONF}/${IMAGE_TYPE}/${PROJECTNAME}.${IMAGE_TYPE}.elf" || ${MP_JAVA_PATH}java -jar "${thisMakefileDir}ezbl_tools.jar" --blobber -artifact="dist/${CONF}/${IMAGE_TYPE}/${PROJECTNAME}.${IMAGE_TYPE}.elf"
	@test "dist/${CONF}/${IMAGE_TYPE}/${PROJECTNAME}.${IMAGE_TYPE}.unified.hex" -nt "dist/${CONF}/${IMAGE_TYPE}/${PROJECTNAME}.${IMAGE_TYPE}.unified.bl2" && ${MP_JAVA_PATH}java -jar "${thisMakefileDir}ezbl_tools.jar" --blobber -artifact="dist/${CONF}/${IMAGE_TYPE}/${PROJECTNAME}.${IMAGE_TYPE}.unified.hex" || true
...
ifneq (,${filter default uart,${CONF}})   # Check if "default" or "uart" MPLAB project build profile is used
	...
	${MP_JAVA_PATH}java -jar "${thisMakefileDir}ezbl_tools.jar" --communicator -com=COM1 -baud=230400 -timeout=1100 -artifact="${DISTDIR}/${PROJECTNAME}.${IMAGE_TYPE}"
	...
	${MP_JAVA_PATH}java -jar "${thisMakefileDir}ezbl_tools.jar" --communicator -com=I2C -i2c_address=0x60 -baud=400000 -timeout=1100 -artifact="${DISTDIR}/${PROJECTNAME}.${IMAGE_TYPE}"
